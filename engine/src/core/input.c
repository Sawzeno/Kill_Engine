...
typedef struct  KeyboardState{
  b32   keys[KEYS_MAX_KEYS];
}KeyboardState;

typedef struct  MouseState{
  i16   x;
  i16   y;
  b32   buttons[BUTTON_MAX_BUTTONS];
}MouseState;
...
void inputProcessKey(keys key, b32
                     pressed)//
{
  TRACEEVENT;
...
void inputProcessButton(buttons button, b32
                        pressed)//
{
  TRACEEVENT;
...
b32
inputWasMouseUp(buttons button)//
{
  TRACEEVENT;
